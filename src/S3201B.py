...
# Программа для чтения информации из файла pickle, взаимодействия с пользователем, поиска кратчайшего пути по графу и вывода результатов
    ...
    while True:
        ...
        for x in s: # Перебираем все вершины из словаря следующих вершин s (до которых знаем путь)
            if x not in u: # При этом перебираем только вновь добавленные возможные станции, и не перебираем уже обработанные, находящиеся в множестве u
                if n == None: # Если это первая вершина, которую обрабатываем на данной итерации, разбираем пару id_станции : время_пути_до_нее на две переменные n и m
                    n, m = x, s[x]
                else: # Если это уже не первая вершина, которую обрабатываем на данной итерации, сравниваем время пути до нее с минимальным временем пути для данной итерации (значение переменной m)
        ...
        if n == nst2: # Если достигли станцию назначения
            result = [nst2]
            x = nst2
            while x != nst1: # Проходим по словарю p, пока не дойдем до начальной станции, и добавляем в список result все станции, через которые прошли для достижения конечной станции
                x = p[x]
